...
AWS 개념을 단일 문서 목록으로만 제공하지 않고, 초보자가 따라갈 수 있는 추천 학습 경로와 자유 탐색 문서를 함께 제공한다.

추천 학습 경로는 "처음 무엇을 배워야 하는지", "왜 이 순서가 필요한지", "어떤 시나리오와 아키텍처 연습으로 확인해야 하는지"를 설명한다. 자유 탐색은 사용자가 특정 AWS 주제나 운영 이슈를 바로 찾아볼 수 있는 문서 목록이다.

## 학습 경로 구조

학습 콘텐츠는 다음 계층으로 구성한다.
...
- `LearningPath`: 초보자 온보딩, 네트워크 기초, 장애 대응처럼 하나의 학습 목적을 가진 추천 커리큘럼이다.
- `LearningModule`: 경로 안에서 한 번에 학습할 수 있는 단위다. 모듈은 학습 목표, 선행 지식, 관련 문서, 관련 시나리오, 관련 아키텍처 연습을 가진다.
- `LearningDocument`: EC2, VPC, ALB, RDS 같은 개별 개념과 실무 주의점을 설명하는 문서다. 하나의 문서는 여러 모듈에서 재사용될 수 있다.

## 초보자 학습 흐름

초기 추천 경로는 백엔드 개발자가 AWS 운영 구조를 이해하는 순서로 구성한다.

1. 단일 서버를 외부에 배포한다.
2. VPC, subnet, Security Group으로 네트워크 경계를 이해한다.
3. ALB와 private subnet으로 외부 진입과 내부 서버를 분리한다.
4. Auto Scaling과 Health Check로 트래픽 증가와 인스턴스 장애에 대응한다.
5. RDS, Read Replica, Redis로 데이터 저장소 부하와 정합성 trade-off를 이해한다.
6. 사용자 작성 아키텍처로 배운 구조를 직접 구성하고 검증한다.

각 단계는 문서를 읽는 것에서 끝나지 않고, 관련 시나리오 실행 또는 아키텍처 작성 연습으로 이어져야 한다.

## 추천 경로와 자유 탐색의 구분

- 추천 학습 경로는 초보자를 위한 기본 진입점이다. 순서, 학습 이유, 선행 조건, 다음 행동을 제공한다.
- 자유 탐색은 특정 주제를 바로 확인하려는 사용자를 위한 보조 진입점이다. 카테고리와 난이도 필터를 유지하되, 문서가 어떤 학습 경로와 모듈에서 쓰이는지 함께 보여준다.
- 시나리오와 아키텍처 빌더는 두 흐름 모두에서 접근 가능해야 한다. 추천 경로에서는 모듈 완료 활동으로 연결하고, 자유 탐색에서는 관련 실습으로 연결한다.
...
| description | 경로 설명 |
| targetLevel | 대상 수준 |
| learningGoal | 경로 전체 학습 목표 |
| recommended | 기본 추천 경로 여부 |
| orderIndex | 노출 순서 |
| moduleIds | 포함된 모듈 ID 목록 |
...
| title | 제목 |
| description | 모듈 설명 |
| learningGoals | 모듈 단위 학습 목표 목록 |
| prerequisites | 선행 지식 또는 선행 모듈 |
| orderIndex | 경로 안의 학습 순서 |
...
| relatedModuleIds | 이 문서를 포함하는 모듈 ID 목록 |
| relatedScenarioIds | 관련 시나리오 ID 목록 |
| relatedArchitecturePracticeIds | 관련 아키텍처 연습 ID 목록 |
...
추천 학습 경로 목록을 조회한다. 기본 응답은 초보자에게 먼저 보여줄 경로를 `orderIndex` 기준으로 반환한다.
...
경로 설명, 포함 모듈, 모듈별 학습 목표, 선행 조건, 관련 문서, 관련 시나리오, 관련 아키텍처 연습을 함께 반환한다.
...
문서 목록은 자유 탐색 화면에서 사용한다. 카테고리, 난이도, 관련 모듈 정보를 함께 제공해 사용자가 현재 문서가 추천 경로의 어느 위치에 있는지 확인할 수 있게 한다.
